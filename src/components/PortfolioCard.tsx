  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-4 text-ivory sm:p-5">
        <p className="text-[0.64rem] font-semibold uppercase tracking-[0.3em] text-gold/90">{safeImage.folderLabel}</p>
        <h3 className="mt-2 font-serif text-2xl leading-tight">{safeImage.title}</h3>
        <p className="mt-2 line-clamp-2 text-xs leading-5 text-ivory/78">{safeImage.tags.join(' · ')}</p>
      </div>
    </motion.button>
  );
}